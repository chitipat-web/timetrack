// =============================================================
// RUDY · Service Worker v253
// Cache: rudy-static-v253, firebase-v253
// Build: 2026-09-25 · v253 — team page shows work/OT as hours+minutes instead of decimal hours
// =============================================================

const STATIC_CACHE  = 'rudy-static-v253';
const FIREBASE_CACHE = 'firebase-v253';
const RUNTIME_CACHE = 'rudy-runtime-v253';
...
const PRECACHE_URLS = [
  ...
  './icon-192.png',
  './icon-512.png'
  // NOTE: splash.mp4 intentionally NOT precached (v253). iOS Safari needs
...
function shouldBypass(url) {
  try {
    ...
    for (const host of BYPASS_HOSTS) {
      // v253: exact or subdomain match only. The old `.includes(host)` substring
...
self.addEventListener('install', (event) => {
  console.log('[SW v253] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v253] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v253] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v253] Deleting old cache:', name);
...
self.addEventListener('fetch', (event) => {
  ...
  // STEP 3.4: VIDEO / RANGE REQUESTS → BYPASS (v253 iOS splash fix).
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v253' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v253] Push parse fail:', e);
  }
});
...
console.log('[SW v253] Loaded — AI quota-friendly retry');
