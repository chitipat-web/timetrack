// =============================================================
// RUDY · Service Worker v173
// Cache: rudy-static-v173, firebase-v173
// Build: 2026-05-19 · Dime-style minimal splash with letter stagger animation
// =============================================================

const STATIC_CACHE  = 'rudy-static-v173';
const FIREBASE_CACHE = 'firebase-v173';
const RUNTIME_CACHE = 'rudy-runtime-v173';
...
self.addEventListener('install', (event) => {
  console.log('[SW v173] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v173] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v173] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v173] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v173' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v173] Push parse fail:', e);
  }
});
...
console.log('[SW v173] Loaded — Dime-style splash');
