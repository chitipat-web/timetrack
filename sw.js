// =============================================================
// RUDY · Service Worker v186
// Cache: rudy-static-v186, firebase-v186
// Build: 2026-05-19 · Splash spiral with VISIBLE glyph rotation (from video)
// =============================================================

const STATIC_CACHE  = 'rudy-static-v186';
const FIREBASE_CACHE = 'firebase-v186';
const RUNTIME_CACHE = 'rudy-runtime-v186';
...
self.addEventListener('install', (event) => {
  console.log('[SW v186] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v186] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v186] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v186] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v186' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v186] Push parse fail:', e);
  }
});
...
console.log('[SW v186] Loaded — Splash spiral + glyph rotation');
