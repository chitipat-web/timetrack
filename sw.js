// =============================================================
// RUDY · Service Worker v107
// Cache: rudy-static-v107, firebase-v107
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v107';
const FIREBASE_CACHE = 'firebase-v107';
const RUNTIME_CACHE = 'rudy-runtime-v107';
...
self.addEventListener('install', (event) => {
  console.log('[SW v107] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v107] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v107] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v107] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v107' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v107] Push parse fail:', e);
  }
});
...
console.log('[SW v107] Loaded — AAA Effects update with full iOS bypass list');
