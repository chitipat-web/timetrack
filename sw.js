// =============================================================
// RUDY · Service Worker v96
// Cache: rudy-static-v96, firebase-v96
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v96';
const FIREBASE_CACHE = 'firebase-v96';
const RUNTIME_CACHE = 'rudy-runtime-v96';
    ...
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v96] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v96] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v96' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v96] Push parse fail:', e);
  }
});
...
console.log('[SW v96] Loaded — AAA Effects update with full iOS bypass list');
