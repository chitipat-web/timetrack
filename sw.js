// =============================================================
// RUDY · Service Worker v154
// Cache: rudy-static-v154, firebase-v154
// Build: 2026-05-17 · Stage-2f (stat cards into glass system)
// =============================================================

const STATIC_CACHE  = 'rudy-static-v154';
const FIREBASE_CACHE = 'firebase-v154';
const RUNTIME_CACHE = 'rudy-runtime-v154';
    ...
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v154] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v154] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v154' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v154] Push parse fail:', e);
  }
});
...
console.log('[SW v154] Loaded — Stage-2f stat cards glass with full iOS bypass list');
