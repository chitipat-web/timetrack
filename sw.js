// =============================================================
// RUDY · Service Worker v97
// Cache: rudy-static-v97, firebase-v97
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v97';
const FIREBASE_CACHE = 'firebase-v97';
const RUNTIME_CACHE = 'rudy-runtime-v97';
    ...
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v97] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v97] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v97' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v97] Push parse fail:', e);
  }
});
...
console.log('[SW v97] Loaded — AAA Effects update with full iOS bypass list');
