// =============================================================
// RUDY · Service Worker v99
// Cache: rudy-static-v99, firebase-v99
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v99';
const FIREBASE_CACHE = 'firebase-v99';
const RUNTIME_CACHE = 'rudy-runtime-v99';
...
self.addEventListener('install', (event) => {
  console.log('[SW v99] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v99] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v99] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v99' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v99] Push parse fail:', e);
  }
});
...
console.log('[SW v99] Loaded — AAA Effects update with full iOS bypass list');
