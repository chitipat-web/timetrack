// =============================================================
// RUDY · Service Worker v111
// Cache: rudy-static-v111, firebase-v111
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v111';
const FIREBASE_CACHE = 'firebase-v111';
const RUNTIME_CACHE = 'rudy-runtime-v111';
...
self.addEventListener('install', (event) => {
  console.log('[SW v111] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v111] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v111] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v111] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v111' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v111] Push parse fail:', e);
  }
});
...
console.log('[SW v111] Loaded — AAA Effects update with full iOS bypass list');
