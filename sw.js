// =============================================================
// RUDY · Service Worker v198
// Cache: rudy-static-v198, firebase-v198
// Build: 2026-05-19 · Iris Aperture splash logo — round, 6 petals + luminous core
// =============================================================

const STATIC_CACHE  = 'rudy-static-v198';
const FIREBASE_CACHE = 'firebase-v198';
const RUNTIME_CACHE = 'rudy-runtime-v198';
...
self.addEventListener('install', (event) => {
  console.log('[SW v198] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v198] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v198] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v198] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v198' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v198] Push parse fail:', e);
  }
});
...
console.log('[SW v198] Loaded — Iris Aperture logo');
