// =============================================================
// RUDY · Service Worker v88
// Cache: rudy-static-v88, firebase-v88
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v88';
const FIREBASE_CACHE = 'firebase-v88';
const RUNTIME_CACHE = 'rudy-runtime-v88';
...
self.addEventListener('install', (event) => {
  console.log('[SW v88] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v88] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v88] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v88' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v88] Push parse fail:', e);
  }
});
...
console.log('[SW v88] Loaded — AAA Effects update with full iOS bypass list');
