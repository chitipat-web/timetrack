// =============================================================
// RUDY · Service Worker v170
// Cache: rudy-static-v170, firebase-v170
// Build: 2026-05-19 · Drag-to-reorder reminders
// =============================================================

const STATIC_CACHE  = 'rudy-static-v170';
const FIREBASE_CACHE = 'firebase-v170';
const RUNTIME_CACHE = 'rudy-runtime-v170';
...
self.addEventListener('install', (event) => {
  console.log('[SW v170] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v170] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v170] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v170] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v170' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v170] Push parse fail:', e);
  }
});
...
console.log('[SW v170] Loaded — Drag-to-reorder reminders');
