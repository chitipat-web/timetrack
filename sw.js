// =============================================================
// RUDY · Service Worker v122
// Cache: rudy-static-v122, firebase-v122
// Build: 2026-05-14 · Fix check-in/out double-tap bug (Goal / Reminders / Break)
// =============================================================

const STATIC_CACHE  = 'rudy-static-v122';
const FIREBASE_CACHE = 'firebase-v122';
const RUNTIME_CACHE = 'rudy-runtime-v122';
...
self.addEventListener('install', (event) => {
  console.log('[SW v122] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v122] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v122] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v122] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v122' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v122] Push parse fail:', e);
  }
});
...
console.log('[SW v122] Loaded — Fix check-in/out double-tap bug with full iOS bypass list');
