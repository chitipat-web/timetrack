// =============================================================
// RUDY · Service Worker v120
// Cache: rudy-static-v120, firebase-v120
// Build: 2026-05-14 · Phase D Liquid Glass (opt-in) (Goal / Reminders / Break)
// =============================================================

const STATIC_CACHE  = 'rudy-static-v120';
const FIREBASE_CACHE = 'firebase-v120';
const RUNTIME_CACHE = 'rudy-runtime-v120';
    ...
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v120] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v120] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v120' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v120] Push parse fail:', e);
  }
});
...
console.log('[SW v120] Loaded — Phase D Liquid Glass (opt-in) with full iOS bypass list');
