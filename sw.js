// =============================================================
// RUDY · Service Worker v167
// Cache: rudy-static-v167, firebase-v167
// Build: 2026-05-19 · Pull-to-refresh threshold haptic + success haptic + ready state
// =============================================================

const STATIC_CACHE  = 'rudy-static-v167';
const FIREBASE_CACHE = 'firebase-v167';
const RUNTIME_CACHE = 'rudy-runtime-v167';
...
self.addEventListener('install', (event) => {
  console.log('[SW v167] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v167] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v167] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v167] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v167' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v167] Push parse fail:', e);
  }
});
...
console.log('[SW v167] Loaded — Pull-to-refresh threshold haptic');
