// =============================================================
// RUDY · Service Worker v121
// Cache: rudy-static-v121, firebase-v121
// Build: 2026-05-14 · Phase D + Settings toggle (Goal / Reminders / Break)
// =============================================================

const STATIC_CACHE  = 'rudy-static-v121';
const FIREBASE_CACHE = 'firebase-v121';
const RUNTIME_CACHE = 'rudy-runtime-v121';
...
self.addEventListener('install', (event) => {
  console.log('[SW v121] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v121] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v121] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v121] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v121' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v121] Push parse fail:', e);
  }
});
...
console.log('[SW v121] Loaded — Phase D + Settings toggle with full iOS bypass list');
