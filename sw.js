// =============================================================
// RUDY · Service Worker v110
// Cache: rudy-static-v110, firebase-v110
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v110';
const FIREBASE_CACHE = 'firebase-v110';
const RUNTIME_CACHE = 'rudy-runtime-v110';
...
self.addEventListener('install', (event) => {
  console.log('[SW v110] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v110] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v110] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v110] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v110' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v110] Push parse fail:', e);
  }
});
...
console.log('[SW v110] Loaded — AAA Effects update with full iOS bypass list');
