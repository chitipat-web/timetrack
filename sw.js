// =============================================================
// RUDY · Service Worker v176
// Cache: rudy-static-v176, firebase-v176
// Build: 2026-05-19 · Splash radial bloom — letters explode out from centre
// =============================================================

const STATIC_CACHE  = 'rudy-static-v176';
const FIREBASE_CACHE = 'firebase-v176';
const RUNTIME_CACHE = 'rudy-runtime-v176';
...
self.addEventListener('install', (event) => {
  console.log('[SW v176] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v176] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v176] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v176] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v176' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v176] Push parse fail:', e);
  }
});
...
console.log('[SW v176] Loaded — Splash radial bloom');
