// =============================================================
// RUDY · Service Worker v197
// Cache: rudy-static-v197, firebase-v197
// Build: 2026-05-19 · Aurora Liquid Crystal splash logo — letterless aurora ribbons
// =============================================================

const STATIC_CACHE  = 'rudy-static-v197';
const FIREBASE_CACHE = 'firebase-v197';
const RUNTIME_CACHE = 'rudy-runtime-v197';
...
self.addEventListener('install', (event) => {
  console.log('[SW v197] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v197] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v197] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v197] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v197' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v197] Push parse fail:', e);
  }
});
...
console.log('[SW v197] Loaded — Aurora logo');
