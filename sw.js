// =============================================================
// RUDY · Service Worker v93
// Cache: rudy-static-v93, firebase-v93
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v93';
const FIREBASE_CACHE = 'firebase-v93';
const RUNTIME_CACHE = 'rudy-runtime-v93';
    ...
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v93] Precache partial fail (ok):', err);
        });
      })
      ...
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v93] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v93' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v93] Push parse fail:', e);
  }
});
...
console.log('[SW v93] Loaded — AAA Effects update with full iOS bypass list');
