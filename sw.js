// =============================================================
// RUDY · Service Worker v276
// Cache: rudy-static-v276, firebase-v276
// Build: 2026-09-26 · v276 — Audit fixes: late only on first session, per-day averages, Israel time, lunch-break state
// =============================================================

const STATIC_CACHE  = 'rudy-static-v276';
const FIREBASE_CACHE = 'firebase-v276';
const RUNTIME_CACHE = 'rudy-runtime-v276';
...
const PRECACHE_URLS = [
  ...
  './icon-192.png',
  './icon-512.png'
  // NOTE: splash.mp4 intentionally NOT precached (v276). iOS Safari needs
...
function shouldBypass(url) {
  try {
    ...
    for (const host of BYPASS_HOSTS) {
      // v276: exact or subdomain match only. The old `.includes(host)` substring
...
self.addEventListener('install', (event) => {
  console.log('[SW v276] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v276] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v276] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v276] Deleting old cache:', name);
...
self.addEventListener('fetch', (event) => {
  ...
  // STEP 3.4: VIDEO / RANGE REQUESTS → BYPASS (v276 iOS splash fix).
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v276' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v276] Push parse fail:', e);
  }
});
...
console.log('[SW v276] Loaded — AI quota-friendly retry');
