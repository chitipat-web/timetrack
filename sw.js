// =============================================================
// RUDY · Service Worker v109
// Cache: rudy-static-v109, firebase-v109
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v109';
const FIREBASE_CACHE = 'firebase-v109';
const RUNTIME_CACHE = 'rudy-runtime-v109';
...
self.addEventListener('install', (event) => {
  console.log('[SW v109] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v109] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v109] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v109] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v109' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v109] Push parse fail:', e);
  }
});
...
console.log('[SW v109] Loaded — AAA Effects update with full iOS bypass list');
