// =============================================================
// RUDY · Service Worker v108
// Cache: rudy-static-v108, firebase-v108
// Build: 2026-05-11 · AAA Effects update
// =============================================================

const STATIC_CACHE  = 'rudy-static-v108';
const FIREBASE_CACHE = 'firebase-v108';
const RUNTIME_CACHE = 'rudy-runtime-v108';
...
self.addEventListener('install', (event) => {
  console.log('[SW v108] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v108] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v108] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v108] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v108' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v108] Push parse fail:', e);
  }
});
...
console.log('[SW v108] Loaded — AAA Effects update with full iOS bypass list');
