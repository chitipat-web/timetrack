// =============================================================
// RUDY · Service Worker v145
// Cache: rudy-static-v145, firebase-v145
// Build: 2026-05-17 · Stage-1 redesign (readability: bg + cards)
// =============================================================

const STATIC_CACHE  = 'rudy-static-v145';
const FIREBASE_CACHE = 'firebase-v145';
const RUNTIME_CACHE = 'rudy-runtime-v145';
...
self.addEventListener('install', (event) => {
  console.log('[SW v145] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v145] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v145] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v145] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v145' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v145] Push parse fail:', e);
  }
});
...
console.log('[SW v145] Loaded — Stage-1 redesign readability with full iOS bypass list');
