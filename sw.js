// =============================================================
// RUDY · Service Worker v130
// Cache: rudy-static-v130, firebase-v130
// Build: 2026-05-17 · Phase F Health + Logging
// =============================================================

const STATIC_CACHE  = 'rudy-static-v130';
const FIREBASE_CACHE = 'firebase-v130';
const RUNTIME_CACHE = 'rudy-runtime-v130';
...
self.addEventListener('install', (event) => {
  console.log('[SW v130] Installing...');
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        return cache.addAll(PRECACHE_URLS).catch(err => {
          console.warn('[SW v130] Precache partial fail (ok):', err);
        });
      })
...
self.addEventListener('activate', (event) => {
  console.log('[SW v130] Activating...');
  event.waitUntil(
    Promise.all([
      caches.keys().then((names) => {
        return Promise.all(
          names
            ...
            .map((name) => {
              console.log('[SW v130] Deleting old cache:', name);
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_VERSION') {
    if (event.ports[0]) event.ports[0].postMessage({ version: 'v130' });
    return;
  }
});
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    console.warn('[SW v130] Push parse fail:', e);
  }
});
...
console.log('[SW v130] Loaded — Phase F Health + Logging with full iOS bypass list');
