...
const STATIC_CACHE = ‘rudy-static-v80’;
const FIREBASE_CACHE = ‘rudy-firebase-v80’;
...
console.log(’[SW v80] Installing…’);
event.waitUntil(
Promise.all([
caches.open(STATIC_CACHE).then(cache => {
console.log(’[SW v80] Caching static assets’);
return cache.addAll(STATIC_ASSETS).catch(err => {
console.log(’[SW v80] Static cache error:’, err);
});
}),
caches.open(FIREBASE_CACHE).then(cache => {
console.log(’[SW v80] Caching Firebase SDK + Fonts’);
return Promise.all(
[…FIREBASE_ASSETS, …FONT_ASSETS].map(url =>
cache.add(url).catch(err => console.log(’[SW v80] Cache error:’, url, err))
)
);
}),
]).then(() => {
console.log(’[SW v80] Installed — skipping waiting’);
...
console.log(’[SW v80] Activating — cleaning ALL old caches’);
event.waitUntil(
(async () => {
const allKeys = await caches.keys();
console.log(’[SW v80] Found caches:’, allKeys);

```
  const deletions = allKeys
    .filter(key => !CURRENT_CACHES.includes(key))
    .map(key => {
      console.log('[SW v80]   x Deleting:', key);
      return caches.delete(key);
    });

  await Promise.all(deletions);
  console.log('[SW v80] Cache cleanup complete. Active:', CURRENT_CACHES);

  await self.clients.claim();

  const clients = await self.clients.matchAll({ type: 'window' });
  console.log('[SW v80] Notifying ' + clients.length + ' client(s) to reload');
...
console.log(’[SW v80] Manual skipWaiting’);
self.skipWaiting();
}
...
console.log(’[SW v80] PURGE_ALL: deleted’, keys.length, ‘caches’);
if (event.source) {
...
console.log(’[SW v80] Push received’);
...
console.log(’[SW v80] Notification click’);
