...
- **Changing colors/backgrounds?** Use the `theme-vars` skill — every hardcoded `color:#fff` or `rgba()` is a future dark-mode bug.
...
Run the `deploy` skill. The three-file rule (index.html + sw.js + version.json) is iron-clad.
